...
metadata {
	definition (
		name: "GreenWave PowerNode 6 Zooz Frankenstein VER 2.0", 
		...
		*/
		...
		//fingerprint manufacturer: "027A", prod: "A000", model: "A004", deviceJoinName: "Zooz Power Strip VER 2.0"
	     fingerprint manufacturer: "0099", prod: "0003", model: "0004", deviceJoinName: "GreenWave PowerNode 6 Zooz Frankenstein VER 2.0"
    }
...
private getAutoOffEnabledParams() {
	def params = []
	def ch = 1
	[6, 10, 14, 18, 22, 26].each {//added six value 26
...
private getAutoOffIntervalParams() {
	def params = []
	def ch = 1
	[7, 11, 15, 19, 23, 27].each {//added six value 27
...
private getAutoOnEnabledParams() {
	def params = []
	def ch = 1
	[8, 12, 16, 20, 24, 28].each {//added six value 28
...
private getAutoOnIntervalParams() {
	def params = []
	def ch = 1
	[9, 13, 17, 21, 25, 29].each {//added six value 29
...
private getManualControlParam() {
	return getParam(30, "Manual Control", 1, 1, enabledOptions) //26
}

private getLedIndicatorModeParam() {
	return getParam(31, "LED Indicator Mode", 1, 1, [0:"LED On When Switch Off", 1:"LED On When Switch On", 2:"LED Always Off"])//27
}

private getMeterReportingEnabledParams() {
	def params = []

	params << getParam(32, "Meter Reporting Enabled (FIRWARE >= 2.2)", 1, 1, enabledOptions)//28
...
private getUsbReportingEnabledParam() {
	return getParam(38, "USB Reporting Enabled (FIRWARE >= 2.2)", 1, 1, enabledOptions)//34
}
